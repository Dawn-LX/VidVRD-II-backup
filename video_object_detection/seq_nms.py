...
def seq_nms(boxes, scores, labels=None, linkage_threshold=0.5, nms_threshold=0.3, score_metric='avg'):
    ...
    sequences = _seq_nms(box_graph, boxes, scores, nms_threshold, score_metric=score_metric)
...
def delete_sequence(sequence_to_delete, sequence_frame_index, scores, boxes, box_graph, suppress_threshold=0.3):
    ...
    for i,box_idx in enumerate(sequence_to_delete):
        ...
        for delete_idx in deletes:
            scores[sequence_frame_index+i][delete_idx]=0
...
def _seq_nms(box_graph, boxes, scores, nms_threshold, score_metric='avg'):
    ...
    while True: 
        ...
        if len(best_sequence) <= 1 or best_score <= 0:
            break 
        # best_score = rescore_sequence(best_sequence, scores, sequence_frame_index, best_score, score_metric=score_metric)
        sequences.append((sequence_frame_index, best_sequence, best_score))
